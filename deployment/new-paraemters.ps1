...
function New-ParameterFile {
    ...
    process {
        [ParameterFileGenerator]::new($Path).GenerateParameterFile($OnlyMandatoryParameter) | ConvertTo-Json -Depth 100
...
Get-ChildItem ./referenceImplementations/core/managementGroupTemplates/policyAssignments/platform -Recurse -Filter microsoft.authorization_policyassignments*.json |% {

    $filename = Join-Path $_.Directory -ChildPath $($_.BaseName + ".parameters.alpha$($_.Extension)")

    Write-Host $filename
    New-ParameterFile -Path $_.FullName > $filename
}